...
query_pairs = {
    "license_code_authority": {
        "inline": (
            "MATCH (w:Work {inline_license:'cc-by', "
            "inline_license_authority:'Creative Commons'}) "
            "RETURN count(w) AS c"
        ),
        "5gnf": (
            "MATCH (w:Work)-[:HAS_TRAIT]->(l:LicenseTrait "
            "{licenseCode:'cc-by', "
            "licenseAuthority:'Creative Commons'}) "
            "RETURN count(w) AS c"
        ),
    },
}
...
raw_output = os.path.join(
    OUTPUT_DIR,
    "structured_license_read_raw_runs.csv",
)
summary_output = os.path.join(
    OUTPUT_DIR,
    "structured_license_read_statistical_summary.csv",
)
